...
save it under the name HaskellBasics.hs (make sure it doesn't have a
.txt extension).

Then start Haskell from the command line and load the file:
...
-- (confusingly, there is also a type Int that is limited to hardware integers)
...
we don't have an argument.  This is considered a cleaner definition than
incr a = plus 1 a
--}
...
{- Note that 'shadow' always returns 200, since the x in the let expression
   shadows the parameter.  There are additional important things to know about
   scoping and overriding -- we will defer this to Scheme (where exactly the
   same issues arise). -}
...
-- or even more clever (Pointfree again):
