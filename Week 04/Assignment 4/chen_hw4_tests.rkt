...
(define mupl-tests 
  (test-suite 
   ...
   ;; evaluate aunit
   (check-equal? (eval-prog (aunit)) (aunit))
   
   ; test isaunit
   (check-equal? (eval-prog (isaunit (aunit))) (int 1))
   (check-equal? (eval-prog (isaunit (int 1))) (int 0))
   (check-equal? (eval-in-env (list (cons "x" (aunit))) (isaunit (var "x"))) (int 1))
   
   ;; test apair
   (check-equal? (eval-prog (apair (int 1) (int 3))) (apair (int 1) (int 3)))
   (check-equal? (eval-prog (apair (add (int 1) (int 2)) (int 4))) (apair (int 3) (int 4)))
   (check-exn #rx"unbound" (lambda () (eval-prog (apair (var "x") (int 4)))))
   (check-equal? (eval-in-env (list (cons "x" (int 3))) (apair (var "x") (int 4))) (apair (int 3) (int 4)))
   (check-equal? (eval-in-env (list (cons "x" (int 5)) (cons "y" (apair (int 6) (int 7))))
                              (apair (var "x") (var "y")))
                 (apair (int 5) (apair (int 6) (int 7))))
   ...
   (check-equal? (eval-in-env (list (cons "x" (apair (int 1) (int 2)))) (fst (var "x"))) (int 1))
   (check-equal? (eval-in-env (list (cons "x" (int 3)) (cons "y" (int 5))) 
                              (fst (apair (add (var "x") (var "y")) (aunit)))) (int 8))
   ...
   (check-equal? (eval-in-env (list (cons "x" (apair (int 1) (int 2)))) (snd (var "x"))) (int 2))
   (check-equal? (eval-in-env (list (cons "x" (int 3)) (cons "y" (int 5))) 
                              (snd (apair (var "y") (add (var "x") (var "y"))))) (int 8))
   ...
   (check-equal? (eval-prog (ifgreater (int 4) (int 4) (int 5) (add (int 1) (int 1))))
                (int 2))
   (check-equal? (eval-in-env (list (cons "x" (int 3))) 
                              (ifgreater (var "x") (int 4) (int 5) (add (int 1) (int 1)))) (int 2))
   (check-equal? (eval-in-env (list (cons "x" (int 3)) (cons "y" (int 4))) 
                              (ifgreater (var "x") (var "x") (int 5) (add (var "y") (int 2)))) (int 6))
   
   ;; test closure
   (check-equal? (eval-prog (closure '() (add (int 1) (int 2)))) (closure '() (add (int 1) (int 2))))
   (check-equal? (eval-prog (closure (list (cons "x" (int 1))) (add (var "x") (int 2))))
                 (closure (list (cons "x" (int 1))) (add (var "x") (int 2))))
   ...
   (check-exn #rx"unbound" 
              (lambda () (eval-prog 
                          (mlet "greater-than-100" 
                                (fun "greater-than-100" "x" (ifgreater (var "x") (int 100) (int 1) (int 0)))
                                (call (var "greater-than-10") (int 101))))))
   ...
   (check-equal? (ifaunit (var "x") (add (int 1) (int 2)) (int 5))
                 (ifgreater (isaunit (var "x")) (int 0) (add (int 1) (int 2)) (int 5)))   
   (check-exn #rx"unbound" (lambda () (eval-prog (ifaunit (var "x") (add (int 1) (int 2)) (int 5)))))
   ...
   (check-exn #rx"MUPL" (lambda () (eval-prog (mlet* (list (cons "x" (aunit)) 
                                                           (cons "y" (add (int 4) (int 3)))) 
                                                     (add (var "x") (var "y"))))))
   ...
   (check-equal? (eval-in-env (list (cons "x" (int 3))) (ifeq (int 3) (var "x") (int 4) (int 5))) (int 4))
   ...
   (check-equal? (eval-prog (call (call mupl-mapAddN (int 7)) (racketlist->mupllist (list))))
                 (aunit))
   (check-exn #rx"MUPL" (lambda () (eval-prog (call (call mupl-mapAddN (int 7)) (int 9)))))
   
   ;; test the mupl-map function -- double the value of each element in the list
   (check-equal? (eval-prog (call (call mupl-map (fun "double" "x" (add (var "x") (var "x")))) 
                                  (racketlist->mupllist (list (int 1) (int 2) (int 3) (int 4)))))
                 (racketlist->mupllist (list (int 2) (int 4) (int 6) (int 8))))
   (check-equal? (eval-prog (call (call mupl-map (fun "double" "x" (add (var "x") (var "x")))) 
                                  (racketlist->mupllist (list))))
                 (aunit))
   (check-exn #rx"MUPL" (lambda () 
                          (eval-prog 
                           (call (call mupl-map (fun "double" "x" (add (var "x") (var "x")))) (int 9)))))
